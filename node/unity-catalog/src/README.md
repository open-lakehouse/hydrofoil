...
- `@open-lakehouse/uc-client` — the UC OpenAPI types the data layer is typed
  against.
- `@rjsf/*` — the JSON-schema form renderer backing `forms/SchemaForm`. The UC
  entity/storage form schemas live in `forms/schemas/` (generated from the UC
  proto by `../uc-client/scripts/gen-form-schemas.mjs`).
