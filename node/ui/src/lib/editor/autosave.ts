...
import { ConflictError, connectFileStore } from "@/lib/files/store";
import { getEntry, isDirty, markSaved } from "./models";
...
export function createAutosave(cb: AutosaveCallbacks): Autosave {
  ...
  async function flush(path: string): Promise<void> {
    ...
    try {
      const stat = await connectFileStore.writeFile(
        path,
