...
// from the Unity Catalog protobuf definitions (the upstream source of truth),
// pinned to the SAME git rev as the `unitycatalog-*` crates in Cargo.toml.
//
// Pipeline:
...
function clean(node) {
  ...
  for (const [key, value] of Object.entries(node)) {
    if (key === "$schema" || key === "$id" || key === "patternProperties") continue;
    out[key] = clean(value);
  }
  ...
  if (Array.isArray(out.anyOf)) {
    ...
    const chosen =
      ...
      branches.find(
        (b) =>
          b && typeof b === "object" && (b.type === "number" || b.type === "integer"),
      ) ??
      branches[0];
    delete out.anyOf;
